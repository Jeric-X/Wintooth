	...
	)
{
	...
	if (SUCCEEDED(hr))
	{
		hr = SHStrDupW(L"连接蓝牙中...", &_rgFieldStrings[SFI_USERNAME]);
	}
	if (SUCCEEDED(hr))
	{
...
HRESULT CCredential::SetSelected(BOOL* pbAutoLogon)
{
	//*pbAutoLogon = FALSE;
	if (_fComplete)
		*pbAutoLogon = FALSE;
	...
	)
{
	...
	if (GetComputerNameW(wsz, &cch))
	{
		...
		//hr = SHStrDupW(L"Jiangruochen426", &pwzProtectedPassword);
		if (SUCCEEDED(hr))
		{
...
//在这处理蓝牙接受的
HRESULT CCredential::SetBluetoothAccount(PWSTR pUsername, PWSTR pPassword)
{
	HRESULT hr = E_FAIL;
	hr = SetStringValue(SFI_USERNAME, pUsername);
	if (SUCCEEDED(hr))
	{
		hr = SetStringValue(SFI_PASSWORD, pPassword);
	}
	if (SUCCEEDED(hr))
	{
		_fComplete = TRUE;
...
HRESULT CCredential::GetUserSid(LPWSTR *sid)
{
	HRESULT hr = E_INVALIDARG;

	if (_pProvider->_pCredProviderUserArray != nullptr)
	{
